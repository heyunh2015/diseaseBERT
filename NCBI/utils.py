...
class BC5CDRProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        # l1 = self._read_data(os.path.join(data_dir, "training_Yun.txt"))
        # l2 = self._read_data(os.path.join(data_dir, "development_Yun.txt"))
        # return self._create_example(l1 + l2, "train")
        return self._create_example(
            self._read_data(os.path.join(data_dir, "train_Yun.txt")), "train"
        )

    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "development_Yun.txt")), "dev"
        )

    def get_trainDevelopment_examples(self, data_dir):
        l1 = self._read_data(os.path.join(data_dir, "train_Yun.txt"))
        l2 = self._read_data(os.path.join(data_dir, "development_Yun.txt"))
        return self._create_example(l1 + l2, "train")
    ...
    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "test_Yun.txt")), "test")
    ...
    def _create_example(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text = line[1]#unicode(line[1], 'utf-8')
            label = line[0]#unicode(line[0], 'utf-8')
...
mask_padding_with_zero=True, sep_token_extra=False):
    ...
    for i, word in enumerate(textlist):
        ...
        for m in range(len(token)):
            if m == 0:
                ...
                labels.append("X")#"X" also different from higgingface offical, where the rest tokens use pad_token_label_id
    ...
    label_ids.append(pad_token_label_id)# I don't think so, should use pad_token_label_id
    #pad_token_label_id: int = nn.CrossEntropyLoss().ignore_index
    ...
    while len(input_ids) < max_seq_length:
        ...
        label_ids.append(pad_token_label_id)# this is obvious not right, 0 reprsents B in his schema
        ntokens.append("**NULL**")
