...
class BinaryProcessor(DataProcessor):
    ...
    def get_maskedLM_examples(self, data_dir):
        return self._create_examples_maskedLM(
            self._read_tsv(os.path.join(data_dir, "extractedQuestionAnswers_total_removeNoisy_maskedLM.txt")), "pretrain")
    ...
    def _create_examples_maskedLM(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(
                InputExampleMaskedLM(guid=guid, text_a=text_a, text_b=text_b, diseaseName=diseaseName, questionType=questionType))
        return examples
...
mask_padding_with_zero=True, sep_token_extra=False):
    ...
    for tokenIndex in range(len(tokens_a)):
        ...
        if token in tokens_diseaseName or token in tokens_questionType:# or 
